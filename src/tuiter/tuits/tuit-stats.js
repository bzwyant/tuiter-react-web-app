	...
	return(
		<div className="d-flex align-content-start justify-content-between mt-3">
			...
			<div>
				...
				}))} className={tuit.liked ? "bi bi-heart-fill text-danger me-2" : "bi bi-heart"}></i>
				{tuit.likes}
			</div>
			<div>
				<i onClick={() => dispatch(updateTuitThunk({
					...tuit, dislikes: (tuit.disliked ? tuit.dislikes - 1 : tuit.dislikes + 1), disliked: !tuit.disliked
				}))} className={tuit.disl ? "bi bi-hand-thumbs-down-fill text-primary me-2" : "bi bi-hand-thumbs-down"}></i>
				{tuit.disliked}
			</div>
			<div>
